...
from src.Utils.utils import get_inputs, get_outputs, string_to_number_class, get_data, heaveside, normalize, \
    get_accuracy, get_confusion_matrix, sigmoid
...
class perceptron:
    ...
    def split(self):
        self.X_train, self.X_test, self.Y_train, self.Y_test = train_test_split(self.X, self.Y, test_size=self.test_rate)

    def train(self, batch=True, normalizer=True):

        weights = rand(self.X_train.shape[1], 1)
        if batch:
            for epoch in range(self.epochs):
                # Hidden = X_train.dot(weights)
                H_output = self.forward(self.X_train, weights)
                Y_output = self.predict(H_output)
                Error = self.Y_train - Y_output

                accuracy, confusion_matrix = self.test(weights, self.X_test, self.Y_test, confusion_matrix=True)
            ...
            for epoch in range(self.epochs):
                ...
                if abs((Y_train - heaveside(self.X_train.dot(weights))).sum()) == 0:
                    # print(epoch)
                    break
